...
PEDDTEMIS >='02.01.2022' AND PEDDTEMIS <= '08.01.2022'),
...
PEDDTEMIS >='02.01.2022' AND PEDDTEMIS <= '08.01.2022') PDT ON P.ID_PEDIDO=PDT.ID_PEDIDO
...
PEDDTEMIS >='02.01.2022' AND PEDDTEMIS <= '08.01.2022') PDT ON PD.ID_PEDIDO=PDT.ID_PEDIDO
...
fillrate_resumo_2_LA <- fillrate_resumo_1_LA %>% 
  ...
  mutate(DATA='02.01.2022 - 08.01.2022')
...
fillrate_calc_filiais_LA <- fillrate_resumo_1_LA %>% 
  ...
  mutate(DATA='02.01.2022 - 08.01.2022') %>% as.data.frame() %>% .[,c(1,3,2,4)] 
...
INNER JOIN (SELECT DISTINCT PD.ID_PEDIDO
              FROM PDPRD PD
               INNER JOIN (SELECT ID_PEDIDO FROM PEDID WHERE PEDDTEMIS >='02.01.2022' AND PEDDTEMIS <= '08.01.2022') PDT ON PD.ID_PEDIDO=PDT.ID_PEDIDO
                INNER JOIN (SELECT PROCODIGO FROM PRODU WHERE GR1CODIGO=2) 
                  PR ON PR.PROCODIGO=PD.PROCODIGO) PRD ON P.ID_PEDIDO=PRD.ID_PEDIDO 
...
fillrate_mp_emp_LA <- union_all(fillrate_mp_matriz_LA,fillrate_mp_joinville_LA) %>% 
  ...
  mutate(DATA='02.01.2022 - 08.01.2022') %>% distinct() %>% as.data.frame()
...
fillrate_mp_emp_resumo_LA <- fillrate_mp_emp_LA %>% group_by(INDICADOR,CHAVE,DESCRICAO_CHAVE) %>% 
  ...
  arrange(desc(QTD)) %>% 
  mutate(DATA='02.01.2022 - 08.01.2022') %>%  as.data.frame()
...
mymail_fillrate_LA <- gm_mime() %>% 
  ...
  gm_subject("RELATORIO FILL RATE LENTES ACABADAS SEMANA 1 - 02/01/2022 - 08/01/2022") %>%
  gm_text_body("Segue Anexo relatorio SEMANA 1 - 02/01/2022 - 08/01/2022") %>% 
