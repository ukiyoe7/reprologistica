...
PEDDTEMIS >=DATEADD(-7 DAY TO CURRENT_DATE) AND PEDDTEMIS < 'TODAY') PDT ON PD.ID_PEDIDO=PDT.ID_PEDIDO
...
filewd_fillrate_resumo_2_LA <-  paste0("C:\\Users\\Repro\\Documents\\R\\LOGISTICA\\LOGISTICA\\BASES\\fillrate_resumo_2_LA","_",format(Sys.Date(),"%d_%m_%y"),".RData")
...
filewd_fillrate_calc_LA<-  paste0("C:\\Users\\Repro\\Documents\\R\\LOGISTICA\\LOGISTICA\\BASES\\fillrate_calc_LA","_",format(Sys.Date(),"%d_%m_%y"),".RData")
...
INNER JOIN (SELECT DISTINCT PD.ID_PEDIDO
              FROM PDPRD PD
               INNER JOIN (SELECT ID_PEDIDO FROM PEDID WHERE PEDDTEMIS >=DATEADD(-7 DAY TO CURRENT_DATE) AND PEDDTEMIS < 'TODAY') PDT ON PD.ID_PEDIDO=PDT.ID_PEDIDO
                INNER JOIN (SELECT PROCODIGO FROM PRODU WHERE GR1CODIGO=2) 
                  PR ON PR.PROCODIGO=PD.PROCODIGO) PRD ON P.ID_PEDIDO=PRD.ID_PEDIDO 
...
filewd_emp_LA <-  paste0("C:\\Users\\Repro\\Documents\\R\\LOGISTICA\\LOGISTICA\\BASES\\fillrate_mp_emp_LA","_",format(Sys.Date(),"%d_%m_%y"),".RData")
...
filewd_emp_resumo_LA <-  paste0("C:\\Users\\Repro\\Documents\\R\\LOGISTICA\\LOGISTICA\\BASES\\fillrate_mp_emp_resumo_LA","_",format(Sys.Date(),"%d_%m_%y"),".RData")
...
#RESUMO
filewd_fillrate_resumo_LA <-  paste0("C:\\Users\\Repro\\Documents\\R\\LOGISTICA\\LOGISTICA\\BASES\\fillrate_resumo_2_LA","_",format(Sys.Date(),"%d_%m_%y"),".RData")
...
filewd_emp_mail_LA <-  paste0("C:\\Users\\Repro\\Documents\\R\\LOGISTICA\\LOGISTICA\\BASES\\fillrate_mp_emp_LA","_",format(Sys.Date(),"%d_%m_%y"),".xlsx")
...
filewd_emp_mail_LA2 <-  paste0("C:\\Users\\Repro\\Documents\\R\\LOGISTICA\\LOGISTICA\\BASES\\fillrate_mp_emp_LA2","_",format(Sys.Date(),"%d_%m_%y"),".csv")

write.csv2(fillrate_mp_emp_LA, file = filewd_emp_mail_LA2,row.names=FALSE)


mymail_fillrate_LA <- gm_mime() %>% 
  gm_to("sandro.jakoska@repro.com.br") %>% 
  gm_from ("comunicacao@repro.com.br") %>%
  gm_subject("RELATORIO FILL RATE LENTES ACABADAS") %>%
